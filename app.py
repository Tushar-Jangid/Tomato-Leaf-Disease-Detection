...
def smart_image_analysis(image):
    """
    Intelligent leaf analyzer:
    1. Validates whether image is readable and not blank/black/white.
    2. Computes color distribution (green, yellow, brown, white, darkness).
    3. Returns realistic prediction probabilities if deep learning model is in fallback mode.
    """
    img_rgb = image.convert('RGB')
    small_img = img_rgb.resize((100, 100))
    arr = np.array(small_img, dtype=np.float32)
    
    # Check image variance to ensure it's not a blank image
    variance = np.var(arr)
    if variance < 20:
    ...
    # Generate realistic heuristic distribution for fallback mode
    scores = {}
...
def preprocess_for_cnn(image):
    """Preprocess image for 128x128 CNN model"""
...
def get_diseases():
    """Return complete disease encyclopedia"""
    return jsonify({
        'success': True,
